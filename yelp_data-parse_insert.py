...
def insert2BusinessTable():  # Parsing and insertion of JSON file entries into the DB - Business
    ...
    with open('./yelp_business.JSON', 'r') as f:
        ...
        try:
            conn = psycopg2.connect("dbname='milestone2db' user='postgres' host='localhost' password='admin'")
        except:
...
def updateBusinessTable():  # Executing SQL queries to update the Business table
    ...
    try:
        ...
        cur.execute("""
            ...
            SELECT Business.business_id, COALESCE(SUM(CheckIn.checkInCount), 0) AS numCheckins, COALESCE(COUNT(DISTINCT Review.review_id), 0) AS reviewcount, COALESCE(AVG(Review.stars), 0) AS reviewrating
